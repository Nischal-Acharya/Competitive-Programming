...
int sumOfAllDigits(int digits) {int sum = 0;while (digits > 0) {sum += digits % 10;digits /= 10;}return sum;}

void solve() {

    int n,total=0, ct=0, mx=0; cin >> n; bool bk = false;
    vector<int> a;
    ...
    // for(int i=0; i<n; i++) if(max==i) max++;

    // if(max%2==0) cout << "YES" << endl;
    // else cout << "NO" << endl;
